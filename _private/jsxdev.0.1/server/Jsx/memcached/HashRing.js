...
include('Jsx/memcached/Bisection.js');
include('Jsx/Util.js');
include('node/crypto.js');
include('node/string_decoder.js');
...
define(function() {
     ...
     * Module dependencies
     */
    var CreateHash = node.crypto.createHash;
    var StringDecoder = node.string_decoder.StringDecoder;
    var Bisection = Jsx.memcached.Bisection;
    ...
    function generateRing(_this) {
        ...
        while (i--) {
            tmp = _this.weights[_this.nodes[i]];
            totalweight += (tmp || 1);
        }
        ...
        for (i = 0; i < len; i++) {
            ...
            if (tmp = _this.weights[node])
                weight = tmp;

            // The factor is based on the weight, the more weight the more space a item will get in our
            // hash ring
            factor = Math.floor((_this.vnode_count * len * weight) / totalweight);

            for (j = 0; j < factor; j++) {

                tmp = _this.hashKey(node + '-' + j);
                for (k = 0; k < 3; k++) {
                    key = _this.hashValue(tmp, function(x) { return x + k * 4 });
                    _this.ring[key] = node;
        ...
        _this.sortedKeys.sort(function(a, b) { return a - b });
    }


    Class('Jsx.memcached.HashRing', null, {
         ...
         * @constructor
         */
        HashRing: function(nodes, weights) {
        ...
        replace: function(oldKey, newKey) {
            ...
            // replace the servers
            Jsx.keys(this.ring).forEach(function(key) {
            ...
            Jsx.keys(this.cache).forEach(function(key) {
