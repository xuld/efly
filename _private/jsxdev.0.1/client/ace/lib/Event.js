...
define(function() {
    ...
    Class('ace.lib.Event', null, null, {
        ...
        */
        ...
        capture: document.documentElement.setCapture ? function(el, eventHandler, releaseCaptureHandler) {
        ...
        function(el, eventHandler, releaseCaptureHandler) {
            function onMouseMove(e) {
                eventHandler(e);
                e.stopPropagation();
            }
        ...
        addMouseWheelListener: function(el, callback) {
            var max = 0;
            var timeoutid = 0;
            function clearMax(){
            	 max = 0;
            ...
            var listener = function(e) {
                if (e.wheelDelta !== undefined) {
                    ...
                    var val = Math.abs(e.wheelDeltaY);
                    if (val > max) 
                        max = val;
                    ...
                    if (max > 5000) {  
                        ...
                        if(val < 5000){ 
                        	...
                        	timeoutid = setTimeout(clearMax, 200);
                        }
                   	}
                    else
                        var factor = 8;
        ...
        addMultiMouseDownListener: function(el, button, count, timeout, callback) {
            var clicks = 0;
            var startX, startY;
            ...
            var listener = function(e) {
                ...
                var isButton = Event.getButton(e) == button;
            ...
            useragent.isOldIE && Event.addListener(el, "dblclick", listener);
        },


        addCommandKeyListener: function(el, callback) {
        ...
        nextTick: window.postMessage ? function(callback, win) {
            win = win || window;
            var messageName = "zero-timeout-message-" + postMessageId;
            Event.addListener(win, "message", function listener(e) {
                if (e.data == messageName) {
                    Event.stopPropagation(e);
                    Event.removeListener(win, "message", listener);
                    callback();
                }
            });
